...
class RegGNN3(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        mu = F.softmax(self.lin(x))
        theta = self.lin2(x)
        # mu = self.lin(x)
        # theta = self.lin2(x)
        pi = F.sigmoid(self.lin3(x))
        # pi = self.lin3(x)
        # x = torch.transpose(x, 2, 1)
        x = x.t()

        mu = mu.t()
        mu = torch.exp(torch.tensor(14))*mu
        theta = theta.t()
        theta = torch.exp(torch.clamp(theta, max=12))
        pi = pi.t()
        pi = torch.special.logit(pi)
        # print(mu)
        # print(theta)
        # print(pi)
        # zinb_dist = dist.negative_binomial.NegativeBinomial(total_count=theta, logits=pi)
        # y = zinb_dist.sample()
        return mu, theta, pi
    ...
    def loss(self, x, mu, theta, pi, eps=1e-8):
