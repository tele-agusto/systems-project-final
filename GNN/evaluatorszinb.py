...
def evaluate_RegGNN(sample_selection=False, shuffle=False, random_state=None,
                    ...
                    device=torch.device('cuda'), num_epoch=150, n_select_splits=10):
    ...
    df_cite_inputs = pd.read_hdf(return_h5_path('train_cite_inputs_raw.h5'))
        ...
        for k in k_list:
            ...
            for epoch in range(num_epoch):
                ...
                for batch in train_loader:
                    ...
                    a = batch.x.to(device, dtype = torch.float32)
                    loss = candidate_model.loss(x=a, mu=out[0], theta=out[1], pi=out[2])
                    if epoch % 50 == 0:
                        print(loss)
                    ...
                    preds.append(out[0].cpu().data.numpy())
                ...
                for batch in test_loader:
                    ...
                    a2 = batch.x.to(device, dtype = torch.float32)
                    # loss2 = candidate_model.loss(out2.reshape(-1, 1), batch.y.to(device).reshape(-1, 1))
                    loss2 = candidate_model.loss(a2, out2[0], out2[1], out2[2])
                    if epoch % 50 == 0:
                      print(f'test:{loss2}')
            ...
            with torch.no_grad():
                ...
                for batch in test_loader:
                    ...
                    # loss = candidate_model.loss(out.reshape(-1, 1), batch.y.to(device).reshape(-1, 1))
                    a = batch.x.to(device, dtype = torch.float32)
                    loss = candidate_model.loss(a, out[0], out[1], out[2])
                    preds.append(out[0].cpu().data.numpy())
